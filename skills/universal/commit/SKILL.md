---
name: commit
description: "Analyzes staged changes, runs verify-implementation, extracts OP issue ID from branch, and generates a structured Conventional Commits message with optional firmware size impact. Use when committing code changes. Do NOT use for amending published commits or force-pushing."
...
1. **Verify Implementation** (Critical Step)
   - Run `/verify-implementation` to check for style violations, missing docs, or known bad patterns (e.g., missing `0.0F` suffix).
