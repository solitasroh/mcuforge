---
name: act-changelog
description: "ALWAYS use when generating a structured changelog from git commit history, grouped by component and change type (feat/fix/refactor), with optional binary size delta. ALWAYS use before release or for release notes generation. Do NOT use for single-commit summaries or individual commit messages (use act-commit), MR creation (use act-gitlab-mr), or code changes."
