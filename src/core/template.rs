...
fn gen_toolchain_cmake(opts: &GenerateOptions) -> String {
    let tc_id = format!("{}-{}", opts.toolchain_vendor, opts.toolchain_version);
    format!(
...
"#,
        vendor = opts.toolchain_vendor.to_uppercase(),
        version = opts.toolchain_version,
        tc_id = tc_id,
    )
}
...
CompileFlags:
  ...
  Remove:
    - -m*                    # Remove host-specific flags clangd doesn't understand
    - --specs=*              # Remove embedded specs (nano.specs, nosys.specs)
...
/// Generate a standalone .clangd config (for init command)
pub fn generate_clangd_config(
    mcu_define: &str,
    core: &str,
    fpu: &str,
) -> String {
    let fpu_flags = match fpu {
        "hard" => "-mfloat-abi=hard, -mfpu=fpv4-sp-d16",
        "softfp" => "-mfloat-abi=softfp, -mfpu=fpv4-sp-d16",
        _ => "-mfloat-abi=soft",
    };
